...
public:
	...
	void tpsBFS() {
		...
		queue<T> q;
        // This is needed since through this we want the nodes whose indegree == 0
		for (auto p : adj) {
		...
		while (!q.empty()) {
			...
			for (auto child : adj[parent]) {
				indegree[child] --;
   // if indegree is 0 that means on whatever HTML was depenedent is printed so go to javascript             
				if (indegree[child] == 0) {
					q.push(child);
				}
			}
    ...
    //visited is an array like DP so it is DP on trees
